    ...
    function walk(current) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (!SUPPORTED.has(ext) && !SPECIAL_FILES.has(name))
                    continue;
                try {
...
// ── Build LLM context ──────────────────────────────────────────────────────────
function buildContext(files, maxChars = 60000) {
    const parts = [];
    let total = 0;
    let skipped = 0;
...
// ── Core review runner ─────────────────────────────────────────────────────────
async function runReview(targetPath, isFolder, ctx) {
    const config = vscode.workspace.getConfiguration('vsec');
    const apiKey = config.get('anthropicApiKey')
        || process.env.ANTHROPIC_API_KEY
        || '';
    const model = config.get('model') || 'claude-haiku-4-5';
    if (!apiKey) {
        vscode.window.showErrorMessage('VSec: No API key. Set vsec.anthropicApiKey in Settings or ANTHROPIC_API_KEY env var.');
        return;
    }
    // Create panel
    const panel = vscode.window.createWebviewPanel('vsecReview', `VSec — ${path.basename(targetPath)}`, vscode.ViewColumn.Beside, { enableScripts: true });
    panel.webview.html = getWebviewHtml();
    const send = (type, data = {}) => panel.webview.postMessage({ type, ...data });
    try {
        let files = [];
        if (isFolder) {
...
function activate(ctx) {
    ctx.subscriptions.push(vscode.commands.registerCommand('vsec.reviewFile', async (uri) => {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('vsec.reviewFolder', async (uri) => {
        if (!uri?.fsPath) {
